...
def append_bot_pbb(bot_detector, dbm_users, users):
    ...
    # of each user found in the tweets
    ...
    for user, bot_detector_pbb in users_pbbs.items():
        dbm_users.update_record(
            {'screen_name': user}, {'bot_detector_pbb': bot_detector_pbb})

def append_interactions_bot_pbb(
        bot_detector, dbm_users, dbm_tweets
        , users, no_interacted_users):
    for user in users:
        ...
        interacted_users_pbbs = (
            bot_detector.compute_bot_probability(interacted_users))
        # And then we store the pbbs into the db
        for interacted_user, bot_detector_pbb \
            ...
            dbm_users.update_record(
                {'screen_name': user['screen_name']}, new_values)
...
def append_user_interactions_bot_ppbs(
        bot_detector, dbm_users, dbm_tweets
        , users, no_users, ignore_list, no_interacted_users):
    """Compute the bot_pbbs for no_users users in the 'users' list
    ...
    for user_number, user in enumerate(users):
        ...
        if 'bot_detector_pbb' in user_record.keys():
            ...
            user_pbb = (
                bot_detector.compute_bot_probability([user_screen_name]))
            dbm_users.update_record(
...
if __name__ == "__main__":
    ...
    # Temporary list. Because some of the accounts may have been deleted
    ...
    if 'bot_detector_pbb' not in user_record.keys():
        # append_bot_pbb(bot_detector, dbm_users, users)
        append_user_interactions_bot_ppbs(
            bot_detector, dbm_users, dbm_tweets
            , users, no_users, ignore_list, no_interacted_users)
    else:
        print(
        ...
        append_user_interactions_bot_ppbs(
            bot_detector, dbm_users, dbm_tweets, users, no_users
            , ignore_list, no_interacted_users)
