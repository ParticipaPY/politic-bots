...
class BotDetector:
    ...
    def __random_account_number(self, data):
        ...
        # random numbers
        if data['screen_name'].isdigit(): #verify if the screen_name is compose only of numbers
            r = 1
        number = ""
        ...
        letter = ""
        for k in data['screen_name']:  # separate numbers of the name to analyze
            if k not in string.digits and k != '_' and k != '-':
                letter = letter + k
            else:
                letter = letter + " "
...
if __name__ == "__main__":
    ...
    users = users + ['AM_1080', 'PrensaSanti2018', 'CESARSANCHEZ553', 'Paraguaynosune', 'Solmelga', 'SemideiOmar',
