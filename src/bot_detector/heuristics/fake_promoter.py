...
def append_bot_detector_pbbs(bot_detector, dbm_users, dbm_tweets, users, NUM_USERS_UPDATE, ignore_list,
    ...
    for user_number, user in enumerate(users):
        ...
        if user_screen_name in ignore_list:
            print('User: {} is in the ignore_list'.format(user))
            continue
        ...
        if 'bot_detector_pbb' in user_record.keys():
            ...
            user_pbb = bot_detector.compute_bot_probability([user_screen_name], False)
        ...
        for interacted_user in sorted(user_interactions, key=lambda interactions:
                                      user_interactions[interactions]['total'], reverse=True):
            ...
            if interacted_user in ignore_list:
                print('User: {} is in the ignore_list'.format(interacted_user))
                continue
            # If pbb already computed for the interacted user,no need to re-compute it
            ...
            if 'bot_detector_pbb' in interacted_user_record.keys(): 
                ...
                new_values = {'interactions.{0}.bot_detector_pbb'.format(interacted_user):
        ...
        interacted_users_pbbs = bot_detector.compute_bot_probability(interacted_users, False)
